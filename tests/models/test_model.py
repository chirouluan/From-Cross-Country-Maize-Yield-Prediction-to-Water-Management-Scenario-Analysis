...
def test_sklearn_model():
    ...
    index = pd.MultiIndex.from_product(
        [[f"US_{i:03d}" for i in range(5)], range(2010, 2020)],
        names=[KEY_LOC, KEY_YEAR],
    )
    feature_cols = [f"f{i}" for i in range(35)]
    feature_values = np.arange(len(index), dtype=float)
    features = pd.DataFrame(
        {column: (feature_values + i) % (i + 7) for i, column in enumerate(feature_cols)},
        index=index,
    )
    yields = pd.DataFrame({KEY_TARGET: 2.0 + 0.1 * feature_values}, index=index)
    full_dataset = Dataset("maize", yields, {KEY_COMBINED_FEATURES: features})
    train_dataset, test_dataset = full_dataset.split_on_years(
        (range(2010, 2018), [2018, 2019])
    )
    ...
    assert {"normalized_rmse", "mape", "r2"} <= set(evaluation_result)
